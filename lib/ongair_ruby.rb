...
module OngairRuby
  ...
  class ClientV1
    ...
    def send_image(phone_number, image_url, content_type='image/jpg', thread=true)
      response = HTTParty.post("#{@base_url}/api/v1/base/send_image", body: {token: @token, phone_number: phone_number, image: image_url, content_type: content_type, thread: thread })
    end

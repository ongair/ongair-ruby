...
module OngairRuby
  ...
  class ClientV1
  	def initialize token, base_url='https://app.ongair.im/'
  		@token = token
      @base_url = base_url
  	end
    ...
    def send_image(phone_number, image_url, thread=true)
      response = HTTParty.post("#{@base_url}/api/v1/base/send_image", body: {token: @token, phone_number: phone_number, image: image_url, thread: thread })
    end
